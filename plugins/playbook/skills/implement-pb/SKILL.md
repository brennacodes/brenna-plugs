...
<steps>
  ...
  <step id="confirm" number="7">
    ...
    <if condition="all-implemented">All items resolved! Consider running `/playbook:update-pb <plan> --status completed`.</if>
    <if condition="has-remaining">Run `/implement-pb <review>` again to continue.</if>
    </completion-message>
  </step>
