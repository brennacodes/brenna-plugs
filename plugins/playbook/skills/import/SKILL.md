...
<purpose>
Import Claude plan documents into playbook in bulk. Discovers plans from `~/.claude/plans/`, displays them with meaningful descriptions so the user can pick and choose, and imports selected plans with proper frontmatter using the same flow as `/capture-plan`.
</purpose>
...
<steps>
  ...
  <step id="import-plans" number="5">
    ...
    <action>For each selected plan, use the same import flow as `/capture-plan`:</action>
    1. Read full content
    2. Generate frontmatter:
       ...
       - `references`: `[]` (empty -- can be added later via `/update`)
  ...
  <step id="confirm" number="7">
    ...
    - `/progress` -- See progress dashboard for all plans
    </completion-message>
  </step>
