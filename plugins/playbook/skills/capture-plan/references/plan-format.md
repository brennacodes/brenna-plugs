...
Plans are imported from `~/.claude/plans/` via `/capture-plan`. The `source_plan` field preserves the original filename for traceability. The import behavior (copy vs. move) is controlled by preferences.
