...
<steps>
  ...
  <step id="diff-state" number="4">
    ...
    Walk the decision tree from `/pitch-htt` to determine what deliverables SHOULD exist for this goal based on current campaign state (stakes, medium, audience, time).
    </phase>
  ...
  <step id="generate-missing" number="6">
    ...
    For each missing deliverable, follow the same generation logic as `/pitch-htt` Step 8.
  ...
  <step id="confirm" number="8">
    ...
    Next steps:
    <if condition="not-rehearsed">- `/heres-the-thing:prep-htt <campaign-id> --qa` -- run a Q&A prep session</if>
    - `/heres-the-thing:outcome-htt <campaign-id>` -- log what happened after delivery
    </completion-message>
  </step>
